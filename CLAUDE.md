...
- **`theme check` passing does not mean the theme will upload.** Theme Check is a static linter: it never validates a setting's *value* against the rules for that setting's *type*. Shopify enforces those server-side on `theme push` / GitHub sync, and a bad value rejects the **entire template file** — on a GitHub-connected theme, silently, so every page using it falls back to the default template. Settings typed `richtext` accept only a small tag/attribute allowlist; `class`, `id` and `data-*` are out (and are stripped at render anyway, so they never worked). Analytics hooks belong in a `custom-liquid` block, whose value is not sanitized — see `page.joshai.json`. `bin/check-richtext-settings.py` catches this locally and in CI; run it before pushing template JSON.
